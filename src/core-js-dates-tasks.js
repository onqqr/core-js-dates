...
function getNextFriday(date) {
  ...
  const nextFriday = (5 + 7 - day) % 7 || 7;
  const newFriday = new Date(date);
  newFriday.setDate(date.getDate() + nextFriday);
  return newFriday;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.ceil(interval / (1000 * 60 * 60 * 24)) + 1;
}
...
function getWeekNumberByDate(date) {
  ...
  const firstDay =
    Math.floor((newDate - startYear) / (24 * 60 * 60 * 1000)) + 1;
  const dayWeek = startYear.getDay() || 7;

  const interval = firstDay + dayWeek;
  const week = Math.ceil((interval - 1) / 7);
...
function getNextFridayThe13th(date) {
  ...
  while (!(result.getDate() === 13 && result.getDay() === 5)) {
